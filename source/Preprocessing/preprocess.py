...
def detect_border_irruglarity(image):
    ...
    blur = cv2.GaussianBlur(gray, (7, 7), 32)
    ret, thresh = cv2.threshold(blur, 120, blur.max(), cv2.THRESH_BINARY_INV+cv2.THRESH_OTSU)
...
def preproc_img():
    ...
    #for i in ocular_images:
        ...
        #img_path = '../POC_MelanomaDetector/Dataset' + str(i)
        #img_for_irr = cv2.imread("../POC_MelanomaDetector/Dataset/ISIC_0024306.jpg")
        img_for_irr = cv2.imread(img_path)
        ...
        gamma_img = np.array(gamma_correction(original_img_rgb, gamma=1.3)*255).astype(np.uint8)  # choose gamma value Anna
        ...
        image_with_irrugular_border = detect_border_irruglarity(img_for_irr)
