...
def threshold(input_img):
    ...
    #edges = cv2.Canny(thresh_img2,150,200)
    img_dilation = cv2.dilate(thresh_img2, kernel, iterations=1)
    #img_erosion = cv2.erode(img_dilation, kernel_err, iterations=1)
...
def segmented_img(blur_img):
    Z = blur_img.reshape((-1,3))
...
def preproc_img():
    ...
    for i in img_index:
        ...
        seg_img = segmented_img(blurred_img)
        bin_img = threshold(seg_img)
        #bin_img = DEM(bin_img)
        external_contours = find_contours(bin_img).astype(np.uint8)
        #external_contours = DEM(external_contours)
        display_img([original_img, gamma_img, blurred_img, bin_img, seg_img, external_contours])
    return bin_img
